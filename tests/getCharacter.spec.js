...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    expect(getCharacter('Arya') && getCharacter('aRya') && getCharacter('ArYa') && getCharacter('ARYA')).toEqual({name: 'Arya Stark', class: 'Rogue', phrases: ['Not today', 'A girl has no name.']}) 
    // Teste se ao passar um nome que não está na tabela, a função retorna undefined.
    expect(getCharacter('Joao Snow') || getCharacter('Joffrey') || getCharacter('Margery')).toBeUndefined();
  });
});
